...
def filter_sidebar(filters, linked_cols):
    ...
    for filter in filters.keys():
        if (filter == 'VENDOR_SBA_REGION'):
            selections[filter] = st.sidebar.multiselect(filter.replace('_',' '), sorted(filters[filter], key=lambda r:int(r.replace('Region ',''))))
        elif (filter != group_by_col) and (filter not in linked_cols.keys()):
