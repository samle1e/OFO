...
def connect_and_get_data ():
    ...
    data_SAM = session.table ("SAM_SENSITIVE_MONTHLY")
...
def global_var():
        ...
        regcols=["UNIQUE_ENTITY_ID",
           ...
           #     'SAM_EXTRACT_CODE',
                ...
                "ENTITY_URL","GOVT_BUS_POC_US_PHONE",
                "GOVT_BUS_POC_EMAIL_",
                "PSC_CODE_STRING",
        ]
...
def set_aside_SBA (data_SBG):
    ...
    set_aside_SBA = setasidestats.groupby(['FISCAL_YEAR','setaside'],as_index=False).sum().round(2)
    ...
    set_aside_SBA = set_aside_SBA.loc[:,['Total SBA Set-Asides','Small Business Set-Asides','8(a) Contracts','Women-Owned Small Bus. Set-Asides','Service Disabled Vet-Owned SB Set-Asides','HUBZone Set-Asides']]
...
def show_quick_stats (SBdolstats, set_aside_SBA, dist_SAM):
    three_yr_SBdol = SBdolstats.drop(['Total Eligible Dollars'],axis=1).iloc[-3:].mean().round(2
